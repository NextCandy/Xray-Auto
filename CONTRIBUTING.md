...
首先，感谢您考虑为本项目做出贡献！正是像您这样的人让开源变得如此美好。

## 如何参与贡献

### 报告 Bug (缺陷)
如果您发现了 Bug，请先检查现有的 GitHub Issues，看看是否已经有人报告过。如果没有，请开启一个新的 Issue，并包含以下内容：
* 清晰且具描述性的标题。
* 复现该行为的详细步骤。
* 预期行为以及实际发生的情况。
* 相关的环境详细信息（操作系统、软件版本等）。

### 建议新功能
如果您有改进本项目的想法，请在编写任何代码之前先开启一个 Issue 进行讨论。请提供该功能的清晰描述，并解释为什么它对用户有用。

### Pull Requests (合并请求)
我们非常欢迎 Pull Requests！要提交代码更改，请遵循以下步骤：
1. Fork 本仓库，并从默认分支创建您的工作分支。
2. 如果您添加了需要测试的代码，请同时添加测试。
3. 确保您的代码遵循项目现有的格式和代码风格。
4. 如果您的更改有需要，请更新相关的文档（例如 `README.md`）。
5. 开启一个 Pull Request，并附带清晰的标题和对更改的全面描述。

## 行为准则
参与本项目即表示您同意为大家维护一个相互尊重且热情的环境。请在交流中保持建设性态度，并遵守我们的 [行为准则](CODE_OF_CONDUCT.md)。
...
## Code of Conduct
By participating in this project, you agree to maintain a respectful and welcoming environment for everyone. Please be constructive in your communication and follow our [Code of Conduct](CODE_OF_CONDUCT.md).
